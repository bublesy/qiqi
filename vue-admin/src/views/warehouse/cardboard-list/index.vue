<template>
  <el-container>
    <el-main>
      ...
      <el-dialog :title="titleType+'纸板入仓单'" :visible.sync="purAddVisible">
        ...
        >
          ...
          <el-table-column prop="taskNumber" label="任务编号" width="140">
            <template slot-scope="scope">
              <el-link type="primary" @click="modifyTask(scope.row)">{{ scope.row.taskNumber }}</el-link>
            </template>
          </el-table-column>
      ...
      <!-- 添加任务编号 -->
      <el-dialog title="添加任务编号" :visible.sync="taskNumberVisible" width="550px">
        <el-table
      ...
      <el-dialog title="请选择" :visible.sync="modifyTaskVisible" width="550px">
        <el-table
...
export default {
  ...
  methods: {
    ...
    // 确定客户成品信息 回调
    modifyTaskConfirm() {
      this.addTableData.push(this.currentRowTow)
