...
public class BillController {
    ...
    @GetMapping("")
    public PageEntity<BillsDTO> getAllBill(@RequestParam(value = "page",defaultValue = "1") Long page,
...
<<<<<<< HEAD
        ...
        Date date2 = null;
        try {
            date1 = DateUtil.parse(startDate, "yyyy-MM-dd");
            date2 = DateUtil.parse(endDate, "yyyy-MM-dd");
        }catch (Exception e){
            e.printStackTrace();
            return null;
        }
...
>>>>>>> hc
        ...
        queryWrapper.eq(!ObjectUtils.isEmpty(customerId),"customer_id",customerId)
                .between(!ObjectUtils.isEmpty(date1)&&!ObjectUtils.isEmpty(endDate),"delivery_date",date1,date2);
